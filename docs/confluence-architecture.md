...
| `IsDrifted(NodeClaim)` | Always `("", nil)` — no **cloud-provider** drift (machine lifecycle belongs to the Rafay platform). Karpenter core's static (`karpenter.sh/nodepool-hash`), requirements and instance-type drift checks still run and precede this hook, so a NodePool template edit — e.g. the broker rendering `expireAfter: Never` on every pool — rolls a live pool's provisioned nodes under its disruption budget; adopted NodeClaims carry no hash annotation and are exempt. |
| `Name()` | `"rafay"` |
